...
export default function Dashboard({ user, token, navigate, updateUser }) {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const [r, p, d, q, s] = await Promise.all([api.getMyReviews(token), api.getProgress(token), api.getDNF(token), api.getQuotes(token), api.getStats(token)]);
        ...
        <div style={{ minHeight: '100vh', paddingTop: '58px' }}>
            ...
            <div style={{ maxWidth: '1240px', margin: '0 auto', padding: `${isMobile ? '32px' : '48px'} ${pad} 24px`, borderBottom: '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px', flexDirection: isMobile ? 'column' : 'row' }}>
                    <h1 style={{ fontSize: isMobile ? '26px' : 'clamp(24px,4vw,42px)' }}>
                        Biblioteca de{' '}
                    ...
                    {stats && (
                        <div style={{ display: 'flex', gap: '10px' }}>
                            {[
                                ...
                                <div key={l} style={{ textAlign: 'center', background: 'var(--accent-sub)', border: '1px solid var(--border-2)', borderRadius: '10px', padding: isMobile ? '10px 12px' : '12px 18px' }}>
                                    <div style={{ fontFamily: "'Syne',sans-serif", fontSize: isMobile ? '18px' : '22px', fontWeight: 800 }}>{v}</div>
            ...
            <div style={{ maxWidth: '1240px', margin: '0 auto', padding: `0 ${pad}` }}>
                <div className="tabs-scroll" style={{ padding: '16px 0 0', gap: '4px' }}>
                    ...
                    <button onClick={() => setShowQuote(true)} style={{ padding: isMobile ? '8px 12px' : '9px 16px', borderRadius: '10px', fontSize: isMobile ? '12px' : '13px', fontWeight: '500', whiteSpace: 'nowrap', background: 'var(--accent)', color: '#fff', border: 'none', cursor: 'pointer', fontFamily: "'Figtree',sans-serif", marginLeft: '4px' }}>🎨 Cita</button>
                </div>
            ...
            {reviews.map((r, i) => (
                ...
                >
                    ...
                    <div style={{ padding: '12px' }}>
                        ...
                        {r.mood_tags && <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '6px' }}>{r.mood_tags.split(',').filter(Boolean).slice(0, 2).map(m => <span key={m} style={{ fontSize: '10px', padding: '2px 6px', background: 'var(--accent-sub)', color: 'var(--accent-2)', borderRadius: '100px', border: '1px solid var(--border)' }}>{m}</span>)}</div>}
                    </div>
                </div>
            ))}
        </div>
