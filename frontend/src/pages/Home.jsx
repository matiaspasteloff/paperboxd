...
export default function Home({ user, token, onAuthClick, navigate }) {
    ...
    useEffect(() => {
        api.getTrending().then(d => {
            const works = (d.works || []).filter(w => w.cover_id).slice(0, isMobile ? 8 : 10);
    ...
    const searchMood = async (mood) => {
        setActiveMood(mood.id); setSearched(true);
    ...
    const pad = isMobile ? '0 16px' : lt(1024) ? '0 24px' : '0 40px';
    ...
    return (
        <div style={{ minHeight: '100vh' }}>
            ...
            <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: `80px ${isMobile ? '20px' : '24px'} 40px`, position: 'relative', overflow: 'hidden' }}>
                ...
                <div className="fadeUp" style={{ animationDelay: '0.35s', width: '100%', maxWidth: '540px' }}>
                    <div style={{ display: 'flex', background: 'var(--surface)', border: '1px solid var(--border-2)', borderRadius: '14px', overflow: 'hidden', boxShadow: '0 8px 40px rgba(0,0,0,0.4)' }}>
                        ...
                        />
                        <button onClick={() => search()} style={{ background: 'var(--accent)', border: 'none', color: '#fff', fontWeight: '700', fontSize: '14px', padding: '0 20px', cursor: 'pointer', fontFamily: "'Figtree',sans-serif", whiteSpace: 'nowrap' }}>
                            {isMobile ? '🔍' : 'Buscar'}
                        </button>
                    </div>
            ...
            {/* ── MOOD FILTERS ── */}
            <section style={{ padding: `0 ${isMobile ? '16px' : lt(1024) ? '24px' : '40px'}`, maxWidth: '1280px', margin: '0 auto', width: '100%' }}>
                <SectionHeader title="Explorar por mood" />
                <div className="tabs-scroll">
                    {MOOD_FILTERS.map(m => (
                        <button key={m.id} onClick={() => searchMood(m)} style={{ padding: '8px 16px', borderRadius: '100px', fontSize: '13px', fontWeight: '500', whiteSpace: 'nowrap', background: activeMood === m.id ? 'var(--accent)' : 'var(--surface)', color: activeMood === m.id ? '#fff' : 'var(--text-dim)', border: `1px solid ${activeMood === m.id ? 'var(--accent)' : 'var(--border-2)'}`, cursor: 'pointer', fontFamily: "'Figtree',sans-serif", transition: 'all 0.15s', boxShadow: activeMood === m.id ? '0 0 16px var(--accent-glow)' : 'none' }}>
                            {m.label}
                        </button>
                    ))}
                </div>
            ...
            {/* ── FEATURED ── */}
            {!searched && (
                <section style={{ padding: `48px ${isMobile ? '16px' : lt(1024) ? '24px' : '40px'}`, maxWidth: '1280px', margin: '0 auto', width: '100%' }}>
            ...
            {/* ── RESULTS ── */}
            {searched && (
                <section ref={resultsRef} style={{ padding: `48px ${isMobile ? '16px' : lt(1024) ? '24px' : '40px'} 80px`, maxWidth: '1280px', margin: '0 auto', width: '100%' }}>
                    {loading ? (
