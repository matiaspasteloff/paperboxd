...
def create_review(
    ...
    if existing:
        ...
        existing = models.Review(
            ...
            mood_tags=review.mood_tags, pace_tag=review.pace_tag, genre=review.genre,
            book_title=review.book_title, cover_url=review.cover_url,
...
async def get_my_reviews(
    ...
    for r in reviews:
        book = (
            {"title": r.book_title, "cover_url": r.cover_url}
            if r.book_title
            else fetched_map.get(r.id, {"title": "Desconocido", "cover_url": None})
        )
        result.append({
            "id": r.id, "rating": r.rating, "review_text": r.review_text,
            "open_library_work_id": r.open_library_work_id, "book": book,
            "mood_tags": r.mood_tags, "pace_tag": r.pace_tag, "genre": r.genre,
            "created_at": r.created_at.isoformat() if r.created_at else None,
        })
    return result